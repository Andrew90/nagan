...
class Reports
{
	public:
		Reports();
		~Reports();
		void ExportAllTubes(TDateTime Begin, TDateTime End,UnicodeString SG ,
				UnicodeString Result , UnicodeString TS , UnicodeString Customer,
				UnicodeString TubeGroup, UnicodeString Defectoscoper, int Last); //Принимает начало и конец периода
	...
	private:
		...
		String AntiGovno(String Res);								// преобразовывает результат из Г, Б, К
};
