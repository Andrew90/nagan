...
Advantech1730::Advantech1730()
{
	...
	bouncePeriod = 3;		// время дребезга - интервал, через который будет проверяться состояние сигнала
	bounceRepeat = 6;		// кол-во повторений дребезга
}
//------------------------------------------------------------------------------
Advantech1730::~Advantech1730()
{
...
//------------------------------------------------------------------------------
bool Advantech1730::BlinkOutBit(byte bit)
{
	if (SetOutBit(bit))
	{
		for( DWORD tt=GetTickCount(); GetTickCount()-tt < blinkTime; Sleep(10) )
...
bool AllSignals::BlinkOutSignal(String SignalName)
{
	Signal s = FindSignal(SignalName, outmap);
	return AdvCards[ s.card ] -> BlinkOutBit( s.channel );
}
//------------------------------------------------------------------------------
